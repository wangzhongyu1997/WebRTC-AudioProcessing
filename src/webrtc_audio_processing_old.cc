...
// #define WEBRTC_AUDIO_PROCESSING_ONLY_BUILD 1
#include <unistd.h>
...
int main(int argc, char **argv) {
    ...
    if (std::string(argv[1]) == "-anc" || std::string(argv[1]) == "-all" ) {
    ...
    }  if (std::string(argv[1]) == "-agc" || std::string(argv[1]) == "-all") {
    ...
    }  if (std::string(argv[1]) == "-aec" || std::string(argv[1]) == "-all") {
        ...
        echo_canell->enable_metrics(true);
